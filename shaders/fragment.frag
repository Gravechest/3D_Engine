...
vec3 blockLight(){
	...
	return (c1.gba+c2.gba+c3.gba+c4.gba+e1.gba+e2.gba+e3.gba+e4.gba+t.gba)/2.0 * (1.0-revertnoise(vec2(wallX,wallY))) * brightness;
}
    ...
    for(int i = 0;i < 64;i++){
        ...
        if(block[0] > 0.0){
			...
			case 8:{
				...
				vec3 li = texelFetch(mapdata,ivec3(x,y,z),0).xyz*10.0;
				if(s != -1.0){
					tr += ang * s;
					FragColor.rgb += li * pow(-dot(tr-0.5,ang),8.0) * 1000.0;
				}
				break;
				}
			case 9:{
